...
int packet2Mp4 (FILE *File, header *head, MP4FileHandle handle,  long data_length)
{
...
int cms_parser( FILE *fp , char *Mp4File)
{
    header head     = { 0 };
    head.width      = 1280;
	head.height     = 720;
        ...
        case CMS_DATA_HEADER:
            if ( ch == '\r' ) {
                ...
                if ( line_buf_ptr == 0 ) {       
                    switch( data_state ) {
                    case CMS_VEDIO:
                        packet2Mp4(fp, &head, handle, data_length);
                        state = CMS_BOUNDARY;
                        break;
                    case CMS_AUDIO:{
                        int i;
